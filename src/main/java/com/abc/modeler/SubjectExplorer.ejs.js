...
//	debugger;
	$(treeIdSub).jstree(true).create_node(node, { nodetype : data.type, path : data.path, text : data.displayName.substring(0, data.displayName.indexOf(".")), type : "file" }, 'last');
...
function com_abc_modeler_SubjectExplorer_NewProcess(newPath) {
	var resource = new MetaworksObject({
		__className : "org.uengine.essencia.portal.EssenciaProcessAdminContainerResource",
		path: newPath
	}, 'body');
...
//			
//		resource.newProcess();	
		com_abc_modeler_SubjectExplorer_NewProcess(node.original.path);
	}
}
...
function com_abc_modeler_SubjectExplorer_Delete(node) { 
	...
	if (ret) {
		if (mw3.getAutowiredObject("org.uengine.essencia.portal.EssenciaEditorPanel") != null &&
			node.original.path == mw3.getAutowiredObject("org.uengine.essencia.portal.EssenciaEditorPanel").resourcePath &&
			mw3.getAutowiredObject("org.uengine.essencia.portal.EssenciaEditorPanel").new == false) {
			mw3.getAutowiredObject("org.uengine.essencia.portal.EssenciaEditorPanel").delete();
			
			window.setTimeout(function() {    	
				com_abc_modeler_SubjectExplorer_NewProcess("codi");
			}, 100);			
		} else {
			var resource = new MetaworksObject({
				__className : "org.uengine.essencia.portal.EssenciaProcessAdminContainerResource",
